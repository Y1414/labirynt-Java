...
public class WallFollower extends AlgorithmThread{
    ...
    public WallFollower(Maze maze, MazeFrame frame) {
        super(maze, frame);
        lock = new Object();
    }
    ...
    private void chooseDirection(){
        ...
        for(int i=0;i<4;i++){
            if (
                    ways[(direction+i)%4] != 'X'
                    && !visited.contains(current.moveForward((direction+i)%4))) {
                direction = (direction + i) % 4;
                return;
            }
        }
        for(int i=0;i<4;i++){
            if (ways[(direction+i)%4] != 'X' && !visitedTwice.contains(current.moveForward((direction+i)%4))) {
                direction = (direction + i) % 4;
                return;
            }
        }
    ...
    public void run() {
        ...
        synchronized (lock) {
            ...
            while (!current.equals(maze.getEnd())){
                ...
                else if (noWay() && !current.equals(maze.getEnd())){
                    frame.changeLabelColor(current.getX(), current.getY(), new Color(255,160,26,255),maze.getWidth() );
                    visitedTwice.add(current);
                }
                frame.revalidate();
